...
namespace OmenModeSwitcher
{
    ...
    internal static class HardwareAccess
    {
        ...
        internal static T Run<T>(Func<T> action)
        {
            lock (Sync)
            {
                ...
                try { return action(); }
                finally { SinceCompletion.Restart(); } // Errors also need recovery time.
            }
        }
    }
    ...
    internal sealed class PawnEcPorts : IEcPorts
    {
        ...
        internal static bool Probe(out uint version, out int error)
        {
            ...
            {
                if (device.IsInvalid) { error = Marshal.GetLastWin32Error(); return false; }
                byte[] output = new byte[4];
                uint returned;
                if (!DeviceIoControl(device, (41394u << 16) | (0x861u << 2),
                    new byte[0], 0, output, 4, out returned, IntPtr.Zero))
                { error = Marshal.GetLastWin32Error(); return false; }
                if (returned != 4) { error = 13; return false; }
        ...
        static extern bool DeviceIoControl(SafeFileHandle device, uint code,
            byte[] input, uint inputSize, byte[] output, uint outputSize,
            out uint returned, IntPtr overlapped);

        internal PawnEcPorts()
        {
            handle = CreateFile(@"\\.\GLOBALROOT\Device\PawnIO", 0xC0000000, 7,
                IntPtr.Zero, 3, 0, IntPtr.Zero);
            if (handle.IsInvalid)
            {
            ...
            try
            {
                byte[] module = File.ReadAllBytes(Path.Combine(
                    AppDomain.CurrentDomain.BaseDirectory, "ec", "LpcACPIEC.bin"));
        ...
        byte[] Call(uint code, byte[] input, int outputLength)
        {
            ...
            uint returned;
            if (!DeviceIoControl(handle, code, input, (uint)input.Length,
        ...
        public byte Read(ushort port)
        {
            ...
            if (value > 255) throw new InvalidDataException("Invalid EC port byte.");
            return (byte)value;
        }
        public void Write(ushort port, byte value) { Invoke("ioctl_pio_write", port, value); }
    ...
    internal sealed class EcReader
    {
        ...
        internal EcReader(IEcPorts ports) { this.ports = ports; }
        ...
        internal PerformanceState ReadPerformance()
        {
            ...
            for (int attempt = 0; attempt < 7; attempt++)
            {
                ...
                if (matches == 3) return new PerformanceState(f8, ec);
                previousF8 = f8;
                previousEc = ec;
                Thread.Sleep(5);
            }
    ...
    internal sealed class PerformanceState
    {
        ...
        internal PerformanceState(byte f8, byte ec) { F8 = f8; EC = ec; }
        internal int Mode
        {
            get
            {
        ...
        internal string Id { get { return Mode == 0 ? "balanced" : Mode == 1 ? "performance" : Mode == 2 ? "comfort" : "unknown"; } }
        internal string Raw { get { return "EC[F8]=0x" + F8.ToString("X2") + ", EC[EC]=0x" + EC.ToString("X2"); } }
    }
    ...
    internal static class HardwareStatus
    {
        ...
        internal static void RequireSupportedBoard()
        {
            ...
            using (var rows = search.Get())
                ...
                using (row)
                    board |= String.Equals(Convert.ToString(row["Product"]).Trim(), "84DB", StringComparison.OrdinalIgnoreCase)
                        && String.Equals(Convert.ToString(row["Manufacturer"]).Trim(), "HP", StringComparison.OrdinalIgnoreCase);
            using (var search = new ManagementObjectSearcher("SELECT SMBIOSBIOSVersion FROM Win32_BIOS"))
            using (var rows = search.Get())
                foreach (ManagementObject row in rows)
                using (row)
                    bios |= String.Equals(Convert.ToString(row["SMBIOSBIOSVersion"]).Trim(), "F.19", StringComparison.OrdinalIgnoreCase);
            if (!board || !bios)
                throw new NotSupportedException("Verified hardware mapping requires HP 84DB BIOS F.19.");
            supported = true;
        }
        ...
        static PerformanceState ReadOnce()
        {
            ...
            using (var ports = new PawnEcPorts())
            using (var mutex = new Mutex(false, @"Global\Access_EC"))
            {
                bool held = false;
                try
                {
                    try { held = mutex.WaitOne(300); }
                    catch (AbandonedMutexException)
                    {
                        held = true;
                        throw new InvalidOperationException("Previous EC client exited during access; refresh again.");
                    }
                    if (!held) throw new TimeoutException("EC is busy with another application.");
        ...
        internal static PerformanceState WaitForMode(byte expected)
        {
            ...
            do
            {
                ...
                try
                {
                    state = Read();
                    lastError = null;
                    if (state.Mode == expected) return state;
                }
