using System; using System.Threading.Tasks; using System.Collections.Generic; using System.Drawing; using System.IO; using System.Linq; using System.Management; using System.Reflection; using System.Runtime.InteropServices; using System.Text; using System.Windows.Forms;
...
namespace OmenModeSwitcher {
 static class HpBios {
  ...
  public static byte[] SetZones(Color[] colors,int[] brightness){byte[] d=GetColors();int n=Math.Min((int)d[0]+1,4);if(n<1)throw new InvalidOperationException("没有可用的键盘分区。");if(colors==null||brightness==null||colors.Length<n||brightness.Length<n)throw new ArgumentException("分区颜色参数不完整。");string f=Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"keyboard-color-backup.bin");if(!File.Exists(f))File.WriteAllBytes(f,d);for(int x=0;x<n;x++){int p=25+x*3,b=Math.Max(0,Math.Min(100,brightness[x]));d[p]=(byte)Math.Round(colors[x].R*b/100.0);d[p+1]=(byte)Math.Round(colors[x].G*b/100.0);d[p+2]=(byte)Math.Round(colors[x].B*b/100.0);}Call("hpqBIOSInt0",0x20009,0x03,d);return d;}
 }
 ...
 sealed class MainForm:Form {
  ...
  bool english,zonesLoaded,installing,hardwareBusy; Label title,lastLabel,mode,color,msg,ecDetails; GroupBox modeBox,kb; Button enableDriver,refresh,def,perf,cool,restoreLegacy,white,red,blue,purple,apply,savePreset,deletePreset,language; TextBox presetName; ComboBox presetList;
  ...
  void SetHardwareControls(bool enabled){enableDriver.Enabled=modeBox.Enabled=apply.Enabled=refresh.Enabled=restoreLegacy.Enabled=enabled;}
  ...
  static bool LegacyEcoPending(){return new[]{"normal-cpu-boost.txt","normal-nvidia-fps.txt","normal-refresh-rate.txt"}.Any(n=>File.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory,n)));}
  async void RestoreLegacy(){if(!BeginHardwareOperation())return;try{bool a,b,c;string detail=TryPart(NvidiaLimiter.LeaveEco,"NVIDIA",out a)+TryPart(DisplayRefresh.LeaveEco,"Display",out b)+TryPart(CpuBoost.LeaveEco,"CPU",out c);restoreLegacy.Visible=LegacyEcoPending();msg.Width=restoreLegacy.Visible?460:610;await RefreshAllCore();if(a&&b&&c)Ok(T("旧版 Eco 设置已恢复。","Old Eco settings restored.")+detail);else Warn(T("部分恢复失败，备份已保留：","Some settings could not be restored; backups retained: ")+detail);}catch(Exception e){Fail(e);}finally{EndHardwareOperation();}}
